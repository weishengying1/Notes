...
class MistalNemo12B(DeepEvalBaseLLM):
    def __init__(
        self,
        model,
        tokenizer
...
# 加载模型和分词器
model_name = "/mnt/project/skyllm/weishengying/Notes/transformers/mistal-nemo"
model = AutoModelForCausalLM.from_pretrained(model_name, torch_dtype=torch.bfloat16, device_map="auto")
tokenizer = AutoTokenizer.from_pretrained(model_name)

mistral_12b = MistalNemo12B(model=model, tokenizer=tokenizer)

from deepeval.benchmarks import MMLU
benchmark = MMLU(n_shots=0)
benchmark.evaluate(model=mistral_12b, batch_size=1)
